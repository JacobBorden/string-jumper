...
let s:previewLine = 0
function! StringJump()
	call fzf#run({'source': 'grep -R --line-number ".*"', 'options':'--preview "bat --color=always --highlight-line '. split(expand('{}'),':')[1] .' '. split(expand('{}'),':')[0] .'"', 'preview-window': ':wrap', 'sink':function('SinkFunction')})
endfunction
